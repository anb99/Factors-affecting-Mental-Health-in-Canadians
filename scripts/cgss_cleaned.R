# Purpose: To clean up the data from the 2017 Canadian General Social Survey 
# dataset obtained from the U of T library. The code to clean this dataset has been obtained from Rohan Alexander and Sam Caetano.
# Authors: Athaliah Biju and Yuxuan Zhao
...
raw_data <- read_csv("AACOq42A.csv")
dict <- read_lines("gss_dict.txt", skip = 18)
#Labels for actual responses
labels_raw <- read_file("gss_labels.txt")

#### Set-up the dictionary ####
# Variable name and variable definitions
...
gss <- 
  gss |>
  select(self_rated_mental_health, province, age, sex, aboriginal, vis_minority, education, income_family, income_respondent, religion_has_affiliation, total_children, own_rent, pop_center, average_hours_worked)
